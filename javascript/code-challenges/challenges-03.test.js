...
const typeNum = (arr) => {
  return arr.filter(num => typeof num === typeof 0);
};
...
const getStatName = (arr, minBaseStat) => {
  const filteredData = arr.filter(el => el.baseStat > minBaseStat);
  return filteredData.map(el => el.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(person => 'children' in person === false);
};
...
const evenOddNumericValues = (arr) => {
  ...
  return filtered.map(num => num % 2 === 0 ? 'even' : 'odd');
};
