...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((item) => {
    ...
    const firstSlice = item.slice(space + 1);

    space = firstSlice.indexOf(' ');
    const final = firstSlice.slice(space + 1);
    result.push(final);
...
const removeEvenValues = (arr) => {
  let l = arr.length;
  for (let i = 0; i < l; i++) {
    let checkEven = arr.pop();
    if (checkEven % 2 !== 0) {
      arr.unshift(checkEven);
    }
  }
};
...
Write a function named totalSumCSV that, given a string of comma-separated values (CSV) as input. (e.g. "1,2,num % 2 === 0
*/
const totalSumCSV = (CSV) => {

  return CSV.split(',').reduce((acc, cur) => +cur + +acc, 0);
};
...
const extractVowels = (str) => {
  const vowels = str.split('').filter(char => /[aeiou]/g.test(char));
