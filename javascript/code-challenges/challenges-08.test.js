...
const matchMonth = (input) => {
  if (input.length === 3) return /^([O-o]ct)$/g.test(input);
  if (input.length === 7) return /^([O-o]ctober)$/g.test(input);
  return false;
};
...
let hangman = (str) => {
  return str.replaceAll(/[aeiouAEIOU]/g, '_');
};
...
const findShells = (str) => {
  const removePuncuation = str.split('.').join('').split(',').join('');
  return removePuncuation.split(' ').filter(word => /ells$/g.test(word));
};
